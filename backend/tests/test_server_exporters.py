...
def test_cadvisor_metrics_are_required_and_normalized():
    samples = parse(
        """
container_cpu_usage_seconds_total{name="/oncall-api",id="abc"} 12
container_memory_working_set_bytes{name="/oncall-api",id="abc"} 1024
"""
    )
    signals, resources = ServerExportersIntegration._container_signals(samples)
    assert signals["container.exporter.up"] == 1
    assert signals["container.metrics.available"] == 1
    assert signals["container.count"] == 1
    assert resources["containers"] == ["/oncall-api"]
...
def test_wrong_prometheus_endpoint_is_rejected():
    ...
    with pytest.raises(ValueError, match="cAdvisor"):
        ServerExportersIntegration._container_signals(parse("some_other_metric 1"))
