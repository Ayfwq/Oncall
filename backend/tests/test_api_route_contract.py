...
EXPECTED = {
    ...
    ("PUT", "/api/servers/{sid}"),
    ("POST", "/api/servers/{sid}/test"),
    ("DELETE", "/api/servers/{sid}"),
    ("POST", "/api/webhooks/alertmanager"),
    ("GET", "/api/prometheus/projects/{pid}/database-metrics"),
    ("GET", "/api/prometheus/projects/{pid}/application-metrics/{source_id}"),
    ...
    ("GET", "/api/projects"),
    ("POST", "/api/projects"),
    ("POST", "/api/projects/onboard/python"),
    ("POST", "/api/projects/onboard/python/test"),
    ("PUT", "/api/projects/{pid}"),
    ("GET", "/api/projects/{pid}"),
    ("DELETE", "/api/projects/{pid}"),
    ("POST", "/api/projects/{pid}/test"),
    ("POST", "/api/projects/{pid}/metrics/discover"),
    ("POST", "/api/projects/{pid}/metrics/apply"),
    ("GET", "/api/projects/{pid}/snapshot"),
    ("GET", "/api/incidents"),
    ("GET", "/api/incidents/{iid}"),
    ("DELETE", "/api/incidents/{iid}"),
    ("POST", "/api/incidents/{iid}/investigate"),
    ("POST", "/api/incidents/{iid}/resolve"),
    ("POST", "/api/incidents/{iid}/conversation"),
    ("GET", "/api/incidents/{iid}/trace"),
    ("GET", "/api/monitoring/metrics"),
    ("GET", "/api/knowledge/documents"),
    ("POST", "/api/knowledge/documents"),
    ("GET", "/api/knowledge/jobs/{jid}"),
    ("POST", "/api/knowledge/documents/{did}/reindex"),
    ("DELETE", "/api/knowledge/documents/{did}"),
    ("POST", "/api/dev/incidents/trigger"),
    ("POST", "/api/dev/incidents/{iid}/recover"),
    ("GET", "/api/settings/readiness"),
    ("GET", "/api/settings/feishu"),
    ("PUT", "/api/settings/feishu"),
    ("GET", "/api/settings/tool-contracts"),
}
    ...
    for node in tree.body:
        ...
        for dec in node.decorator_list:
            ...
            if not isinstance(dec.func.value, ast.Name) or dec.func.value.id != "app":
                continue
            if dec.func.attr.lower() not in {"get", "post", "put", "patch", "delete"}:
                continue
            if dec.args and isinstance(dec.args[0], ast.Constant):
                actual.add((dec.func.attr.upper(), dec.args[0].value))
