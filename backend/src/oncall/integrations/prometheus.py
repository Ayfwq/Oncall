...
class PrometheusIntegration:
    ...
    This class is used only by the one-off onboarding dry-run. Production alert
    decisions are made by Prometheus recording/alerting rules; this probe does not
    persist cursors, samples or incidents.
    """

    name = "prometheus"

    def __init__(
        self, session, project_id, sources: list[MetricsSourceDTO], *, persist_state: bool = True
    ):
    ...
    async def _scrape_source(self, src: MetricsSourceDTO) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(
            timeout=src.scrape_timeout_ms / 1000.0, trust_env=False
        ) as client:
        ...
        for fam in families:
            ...
            has_route = any(
                (
                    src.route_label in s.labels
                    or "handler" in s.labels
                    or "uri" in s.labels
                    or "view" in s.labels
                )
        ...
        return {
            ...
            "meta": {
                ...
                "ok": True,
                "routes": len(routes),
                "process_metrics": process["cpu_seconds_total"] is not None,
            },
        }
    ...
    async def collect(self) -> CollectResult:
        ...
        for src in enabled:
            try:
                ...
                resources["sources"].append(
                    {"name": src.name, "url": src.url, "ok": False, "error": str(exc)}
                )
                continue
            any_ok = True
            ...
            if process_cpu is not None:
                ...
                delta_process_cpu = (
                    float(process_cpu) - previous_process_cpu
                ...
                if start_time is not None:
                    process_uptime_seconds = max(
                        process_uptime_seconds, max(0.0, now - float(start_time))
                    )
            ...
            for handler, r in routes.items():
                ...
                for le, cumulative in r["buckets"].items():
                    ...
                    delta_bucket = (
                        float(cumulative) - previous_bucket if previous_bucket is not None else 0.0
                    )
                    if delta_bucket < 0:
                        bucket_reset = True
            ...
            agg_avail = (
                (1.0 - agg_err_delta / agg_req_delta) * 100.0 if agg_req_delta > 0 else 100.0
            )
            ...
            signals.update(
                {
                    ...
                    "app.http.error_rate": (global_err_delta / global_req_delta)
                    if global_req_delta > 0
                    else 0.0,
    ...
    async def query(self) -> ToolResult:
        ...
        return ToolResult(
            ok=result.ok,
            summary=f"采集 {len(result.resource_signals)} 个路由的 API 指标"
            if result.ok
            else "未能抓取任何指标源",
            data={
                "signals": result.signals,
                "routes": routes,
                "sources": result.resources.get("sources", []),
            },
        )
