...
class ServerExportersIntegration:
    ...
    def _node_signals(samples: list[Any]) -> tuple[dict[str, float], dict[str, Any]]:
        ...
        memory_percent = (
            (1.0 - memory_available / memory_total) * 100.0 if memory_total > 0 else 0.0
        )
        ...
        counters = {
            ...
            "disk_read_bytes": _sum(
                samples, "node_disk_read_bytes_total", skip_label=("device", {"loop0"})
            ),
            "disk_write_bytes": _sum(
                samples, "node_disk_written_bytes_total", skip_label=("device", {"loop0"})
            ),
            "net_rx_bytes": _sum(
                samples, "node_network_receive_bytes_total", skip_label=("device", ignored_devices)
            ),
            "net_tx_bytes": _sum(
                samples, "node_network_transmit_bytes_total", skip_label=("device", ignored_devices)
            ),
        }
        signals = {
        ...
        resources = {
            "cpu_count": len(
                {s.labels.get("cpu") for s in samples if s.name == "node_cpu_seconds_total"}
            ),
    ...
    @staticmethod
    def _container_signals(samples: list[Any]) -> tuple[dict[str, float], dict[str, Any]]:
        container_samples = [sample for sample in samples if sample.name.startswith("container_")]
        if not container_samples:
            raise ValueError(
                "endpoint is reachable but does not contain cAdvisor container metrics"
            )
        container_names = {
            sample.labels.get("name") for sample in container_samples if sample.labels.get("name")
        }
        return (
            {
                "container.exporter.up": 1.0,
                "container.metrics.available": 1.0,
                "container.count": float(len(container_names)),
            },
            {
                "metric_samples": len(container_samples),
    ...
    def _gpu_signals(
        ...
        for sample in samples:
            ...
            if not field:
                continue
            key = (
                sample.labels.get("UUID")
                or sample.labels.get("gpu")
            ...
            row = devices.setdefault(
                ...
                {
                    "id": key,
                    "name": sample.labels.get("modelName")
                    or sample.labels.get("model")
                    or "NVIDIA GPU",
                },
            )
        ...
        signals = {
            ...
            "host.gpu.utilization_percent": sum(
                float(x.get("utilization_percent", 0.0)) for x in rows
            )
            / len(rows),
            "host.gpu.memory_percent": max(float(x.get("memory_percent", 0.0)) for x in rows),
            "host.gpu.temperature_celsius": max(
                float(x.get("temperature_celsius", 0.0)) for x in rows
            ),
    ...
    async def collect(self) -> CollectResult:
        ...
        try:
            ...
            resources["node"] = {
                "ok": True,
                "url": self.server.node_metrics_url,
                "bytes": size,
                **node_resources,
            }
        ...
        try:
            container_samples, size = await self._scrape(self.server.container_metrics_url)
            container_signals, container_resources = self._container_signals(container_samples)
            signals.update(container_signals)
            resources["container"] = {
                "ok": True,
                "url": self.server.container_metrics_url,
                "bytes": size,
                **container_resources,
            }
        except Exception as exc:  # noqa: BLE001
            signals["container.exporter.up"] = 0.0
            detail = _connection_error(exc)
            errors.append(f"cAdvisor: {detail}")
            resources["container"] = {
                "ok": False,
                "url": self.server.container_metrics_url,
                "error": detail,
        ...
        if self.server.gpu_metrics_url:
            try:
                ...
                resources["gpu"] = {
                    "ok": True,
                    "url": self.server.gpu_metrics_url,
                    "bytes": size,
                    **gpu_meta,
                }
            except Exception as exc:  # noqa: BLE001
