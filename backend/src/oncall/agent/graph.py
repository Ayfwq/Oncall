...
_INCIDENT_QUERY_HINTS_BY_ALERT = {
    # These names mirror the centrally rendered Prometheus rules in
    # integrations/prometheus_api.py. Exact matches take precedence over the
    # fallback keyword hints below, so e.g. cAdvisor being down is not treated
    # as a generic container CPU problem.
    "OncallApplicationMetricsDown": "Python 应用 Prometheus /metrics 指标端点不可用 应用可用性 采集故障排查",
    "OncallHostExporterDown": "Node Exporter 服务器指标采集器不可用 主机监控故障排查",
    "OncallApplicationErrorRateHigh": "Python 应用 5xx 错误率过高 应用错误 排查处理方案",
    "OncallApplicationLatencyHigh": "Python 应用接口 P95 延迟过高 响应变慢 排查处理方案",
    "OncallHostMemoryHigh": "服务器内存使用率过高 内存不足 OOM 排查处理方案",
    "OncallHostDiskHigh": "服务器磁盘使用率过高 磁盘空间不足 排查处理方案",
    "OncallDatabaseDown": "PostgreSQL 数据库不可用 数据库连接失败 健康检查 排查处理方案",
    "OncallDatabaseConnectionsHigh": "PostgreSQL 数据库连接使用率过高 连接池耗尽 排查处理方案",
    "OncallDatabaseLockWait": "PostgreSQL 数据库锁等待 阻塞事务 排查处理方案",
    "OncallDatabaseLongTransaction": "PostgreSQL 数据库长事务 pg_stat_activity 排查处理方案",
    "OncallDatabaseReplicationLagHigh": "PostgreSQL 数据库复制延迟过高 主从同步 排查处理方案",
    "OncallContainerMetricsDown": "cAdvisor Docker 容器指标采集器不可用 容器监控故障排查",
    "OncallContainerMemoryHigh": "Docker 容器内存使用率过高 OOM 内存限制 排查处理方案",
    "OncallContainerCpuHigh": "Docker 容器 CPU 使用率过高 高负载 热点进程 排查处理方案",
    # Kept for development smoke tests and future host CPU rule support.
    "OncallHostCpuHigh": "服务器 CPU 使用率过高 高负载 热点进程 排查处理方案",
}


def build_knowledge_query(state: OncallState) -> str:
    """Build a retrieval query from the incident, not from the worker sentence.

    The worker message is an instruction to the Agent ("investigate this
    incident"), not a description of the failure. Searching that instruction
    produces generic or unrelated SOP hits. Incident retrieval must instead
    carry the alert identity and human summary, plus a small set of domain
    terms that make the intent explicit to the hybrid retriever.
    """
    incident = state.get("incident_context") or {}
    current = " ".join(str(state.get("user_message") or "").strip().split())
    anomaly = " ".join(str(incident.get("anomaly_type") or "").strip().split())
    summary = " ".join(str(incident.get("summary") or "").strip().split())
    is_incident = bool(anomaly or summary or state.get("incident_id"))
    if not is_incident:
        return current[:1000]

    # Do not pollute an incident query with the generic monitor instruction.
    generic_monitor_message = current.startswith("请基于当前 Incident")
    parts = [x for x in (anomaly, summary) if x]
    if current and not generic_monitor_message:
        parts.append(current)
    hint = _INCIDENT_QUERY_HINTS_BY_ALERT.get(anomaly)
    if hint:
        parts.append(hint)
    parts.extend(("故障排查", "处理方案", "恢复验证"))
...
class OncallGraphRuntime:
    ...
    def _emit(self, event_type: str, data: dict | None = None) -> None:
        if self.emit:
            try:
                ...
                logger.warning(
                    "emit callback failed", extra={"event_type": event_type}, exc_info=exc
                )
    ...
    def build(self, checkpointer=None):
        ...
        g.add_conditional_edges(
            ...
            {"knowledge": "retrieve_knowledge", "reason": "reason", "clarify": "finalize"},
        )
        g.add_edge("retrieve_knowledge", "reason")
        g.add_conditional_edges(
            "reason", self.route_after_reason, {"tool": "guard_tools", "final": "finalize"}
        )
        g.add_conditional_edges(
            "guard_tools", self.route_after_guard, {"tool": "execute_tools", "final": "finalize"}
        )
        g.add_edge("execute_tools", "record_observations")
        g.add_edge("record_observations", "reason")
        g.add_edge("finalize", "persist_result")
    ...
    async def load_context(self, state: OncallState) -> dict:
        ...
        return {
            **built,
            ...
            "reason_loops": 0,
            "reason_loop_budget": loop_budget,
            "called_tools": [],
            "knowledge_refs": [],
            "knowledge_hits": [],
    ...
    async def route_intent(self, state: OncallState) -> dict:
        ...
        if route.get("intent") == "clarification":
            route["decision"] = {
                "action": "final",
                "rationale": route.get("route_reason", ""),
                "answer": route.get("clarification_question"),
            }
        return route
    ...
    async def retrieve_knowledge(self, state: OncallState) -> dict:
        ...
        if not query:
            return {"knowledge_status": "skipped"}
        ctx = ToolExecutionContext(
            project_id=UUID(state["project_id"]) if state.get("project_id") else None,
            incident_id=UUID(state["incident_id"]) if state.get("incident_id") else None,
        ...
        try:
            ...
            key = "search_knowledge:" + __import__("json").dumps(
                {"query": query, "top_k": 5}, sort_keys=True, ensure_ascii=False
            )
            return {
    ...
    async def reason(self, state: OncallState) -> dict:
        ...
        if loops > state.get("reason_loop_budget", 4) or state.get(
            "tool_calls_used", 0
        ) >= state.get("tool_budget", 5):
            return {
                "reason_loops": loops,
                "exhausted": True,
    ...
    async def guard_tools(self, state: OncallState) -> dict:
        ...
        if name not in ALLOWED_TOOLS or name not in allowed:
            return {
                "pending_tool": None,
                "decision": {"action": "final", "answer": f"工具 {name} 未授权。"},
                "exhausted": True,
            }
        ...
        if call_key in state.get("called_tools", []):
            return {
                "pending_tool": None,
                "decision": {"action": "final", "answer": "没有获得新的证据；停止重复工具调用。"},
                "exhausted": True,
            }
        if state.get("tool_calls_used", 0) >= state.get("tool_budget", 5):
            return {
                "pending_tool": None,
                "decision": {"action": "final", "answer": None},
                "exhausted": True,
            }
        return {"pending_tool": {"name": name, "args": args, "call_key": call_key}}

    def route_after_guard(self, state: OncallState) -> str:
        return "tool" if state.get("pending_tool") else "final"

    async def execute_tools(self, state: OncallState) -> dict:
        p = state["pending_tool"]
        ctx = ToolExecutionContext(
            project_id=UUID(state["project_id"]) if state.get("project_id") else None,
            incident_id=UUID(state["incident_id"]) if state.get("incident_id") else None,
            agent_run_id=UUID(state["run_id"]),
        ...
        try:
            ...
            logger.warning(
                "tool dispatch raised; degrading to failed result",
                extra={"tool_name": p["name"]},
                exc_info=exc,
            )
            self._emit(
                "tool_finished",
                {"tool_name": p["name"], "ok": False, "error": redact_text(str(exc))[:500]},
            )
            failed = ToolResult(
                ok=False,
                summary="工具调用内部错误",
            ...
            return {
                "current_tool_result": failed,
                "tool_calls_used": state.get("tool_calls_used", 0) + 1,
                "called_tools": [*state.get("called_tools", []), p["call_key"]],
            }
        dumped = result.model_dump(mode="json")
        self._emit(
            "tool_finished",
    ...
    async def record_observations(self, state: OncallState) -> dict:
        p = state.get("pending_tool") or {}
        r = state.get("current_tool_result") or {}
        evidence = list(state.get("evidence", []))
        refs = list(state.get("knowledge_refs", []))
        ...
        if p.get("name") == "search_knowledge":
            ...
            item = EvidenceItem(
                ...
                source_tool=p.get("name", "unknown"),
                observed_at=self._parse_observed_at(r.get("observed_at")),
                ...
                try:
                    self.session.add(
                        IncidentEvidence(
                            ...
                            summary=r.get("summary", ""),
                            data={"result": r.get("data") or {}},
                            raw_ref=r.get("source_ref"),
                        )
                    )
        ...
        try:
            return (
                await self.session.scalar(
                    select(Incident.id).where(Incident.id == UUID(incident_id))
                )
                is not None
            )
        except Exception:
            logger.warning(
                "incident_alive check failed", extra={"incident_id": incident_id}, exc_info=True
            )
            return False
    ...
    async def finalize(self, state: OncallState) -> dict:
        ...
        if mode == "investigate":
            ...
            if not diagnosis:
                summaries = [
                    x.get("summary", "") for x in state.get("evidence", []) if x.get("summary")
                ]
                diagnosis = DiagnosisReport(
                    summary="调查在当前预算内收敛",
                    severity=(state.get("incident_context") or {}).get("severity", "warning"),
                    affected_service=(state.get("incident_context") or {}).get("project_name"),
                    symptoms=[(state.get("incident_context") or {}).get("summary", "")],
                    evidence=summaries[-8:],
                    root_cause="当前证据不足以确认唯一根因。",
                    confidence=0.2,
                    remediation=["根据已收集 Evidence 继续人工排查", "必要时重新触发告警调查"],
                    verification=["确认触发指标越过恢复阈值并连续满足恢复次数"],
                    risks=["不要在根因未确认时执行破坏性操作"],
            ...
            try:
                self.session.add(
                    Diagnosis(
                        incident_id=UUID(state["incident_id"]),
                        agent_run_id=UUID(state["run_id"]),
                        structured_json=rep.model_dump(mode="json"),
