...
# A metric/resource word by itself does not imply a project-scoped query.
# For example, "CPU 持续过高怎么排查" is a general operations question and
# must be allowed to use the shared knowledge base without a project.
# Keep this list limited to explicit current-state/query language.
REALTIME_MARKERS = (
    "当前",
    "现在",
    "最近",
...
def classify_intent(
    message: str, *, project_id: str | None, incident_id: str | None, mode: str = "chat"
) -> dict[str, Any]:
    """Return an auditable first-pass route without calling tools or the LLM."""
    text = " ".join(str(message or "").strip().lower().split())
    if incident_id and mode == "investigate":
        return _route(
    ...
    if (
        ...
        and (
            not any(marker in text for marker in OPS_MARKERS)
            or any(marker in text for marker in INCIDENT_MARKERS)
        )
    ):
        return _route(
            "incident_followup", True, True, True, bool(project_id), True, "当前会话关联 Incident"
        )
    if any(marker in text for marker in CASUAL_MARKERS) and not any(
        marker in text for marker in OPS_MARKERS
    ):
    ...
    if realtime:
        ...
        return _route(
            "project_query", True, True, False, True, True, "问题包含项目实时状态或诊断信号"
        )
    if is_ops:
...
def _route(
    ...
    return {
        ...
        "clarification_question": "请先选择或绑定要查询的监控项目。"
