...
class ConversationService:
    ...
    async def create(
        ...
        c = Conversation(
            user_id=user_id, title=title, project_id=project_id, incident_id=incident_id, type=type_
        )
        self.session.add(c)
    ...
    async def list(
        ...
        return list(
            (await self.session.scalars(stmt.order_by(Conversation.updated_at.desc()))).all()
        )
