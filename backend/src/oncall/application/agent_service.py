...
class AgentService:
    ...
    async def run(self,conversation_id:UUID,user_message:str,channel:str='web',mode:AgentMode|None=None,emit=None)->dict:
        ...
        run=AgentRun(mode=mode.value,conversation_id=conversation_id,incident_id=conv.incident_id,status='running',model_profile='default',prompt_version='current');self.session.add(run);await self.session.commit();await self.session.refresh(run)
