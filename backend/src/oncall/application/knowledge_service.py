...
class KnowledgeService:
    ...
    async def upload(self, user_id: UUID, path: Path, title: str | None = None):
        ver = await KnowledgeIngestor(self.session).register_upload(user_id, path, title)
        job = await JobQueue(self.session).enqueue(
            "rag_ingest",
            {"version_id": str(ver.id)},
            idempotency_key=f"rag_ingest:{ver.id}",
            priority=50,
        )
        return ver, job

    async def search(self, query: str):
        return await KnowledgeRetriever().search(query)
