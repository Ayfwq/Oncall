...
class MilvusKnowledgeIndex:
    def __init__(self):
        ...
        self.collection = "oncall_knowledge"
        self._client_conn: Any = None
        ...
        if self._client_conn is None:
            with self._client_lock:
                if self._client_conn is None:
                    ...
                    self._client_conn = MilvusClient(
    ...
    def _ensure_sync(self) -> None:
        with self._ensure_lock:
            ...
            index.add_index("dense", index_type="AUTOINDEX", metric_type="COSINE")
    ...
    async def upsert(self, rows: list[dict]) -> None:
        if not rows:
            return
        await self.ensure()
        await asyncio.to_thread(self._client().upsert, collection_name=self.collection, data=rows)
    ...
    async def delete_document(self, document_id: str) -> None:
        """Remove every indexed version belonging to one logical document."""
        await self.ensure()
        await asyncio.to_thread(
            self._client().delete,
            collection_name=self.collection,
    ...
    async def list_version_ids(self) -> set[str]:
        """Return version ids currently present in Milvus.

        The PostgreSQL tables are the source of truth.  This small inventory
        operation lets the worker remove vectors left behind by deleted or
        superseded document versions and repair a partially lost index.
        """
        await self.ensure()
    ...
    async def dense_search(self, vector: list[float], limit: int = 20) -> list[dict]:
        await self.ensure()
        ...
        return [
            dict(hit.get("entity", {}), id=str(hit.get("id")), score=float(hit.get("distance", 0)))
            for hit in (res[0] if res else [])
        ]

    async def bm25_search(self, query: str, limit: int = 20) -> list[dict]:
        await self.ensure()
        res = await asyncio.to_thread(
            self._client().search,
            collection_name=self.collection,
