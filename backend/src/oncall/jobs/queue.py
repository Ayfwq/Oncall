...
class JobQueue:
        ...
        *,
        ...
        if idempotency_key:
            ...
            if existing:
                return existing
        job = BackgroundJob(
            type=type_, payload=payload, idempotency_key=idempotency_key, priority=priority
        )
    ...
    async def fail(self, job_id: UUID, error: str, retry_delay_seconds: int = 10) -> None:
        ...
        row = (
            await self.session.execute(
                select(BackgroundJob.attempts, BackgroundJob.max_attempts).where(
        ...
        if attempts >= max_attempts:
            ...
            values["available_at"] = datetime.now().astimezone() + timedelta(
                seconds=retry_delay_seconds * max(1, attempts)
            )
        await self.session.execute(
